...
static bool fpread(el_ctx *ctx, void *dest, size_t nb, size_t offset)
{
    ...
    if (f_lseek(&fd, offset))
        return false;
    if (f_read(&fd, dest, nb, &bytesRead) != FR_OK || bytesRead != nb)
        return false;
    return true;
}
...
static bool load_shdr_table(el_ctx *ctx, shdr_table_t *t)
{
    ...
    /* --- DEBUG temporario: confirmar se a leitura em bloco (que
     * cruza fronteira de setor) veio certa --- */
    kprintf("DEBUG strtab_off=%lu strtab_size=%lu\n",
            (unsigned long)strtab_off, (unsigned long)strtab_size);
    kprintf("DEBUG shdrs[0..439] lidos em UM fpread de %lu bytes em offset %lu\n",
            (unsigned long)(shnum * shentsize), (unsigned long)shoff);
    for (uint16_t i = 0; i < shnum; i++) {
        uint8_t *h = t->shdrs + (size_t)i * shentsize;
        uint32_t name_idx = ((uint32_t)h[0]<<24)|((uint32_t)h[1]<<16)|((uint32_t)h[2]<<8)|h[3];
        uint32_t addr     = ((uint32_t)h[0x0C]<<24)|((uint32_t)h[0x0D]<<16)|((uint32_t)h[0x0E]<<8)|h[0x0F];
        uint32_t size     = ((uint32_t)h[0x14]<<24)|((uint32_t)h[0x15]<<16)|((uint32_t)h[0x16]<<8)|h[0x17];
        const char *nm = (name_idx < strtab_size) ? (const char *)(t->strtab + name_idx) : "?";
        kprintf("DEBUG shdr[%2u] name_idx=%3lu name=\"%s\" addr=0x%08lx size=0x%08lx\n",
                i, (unsigned long)name_idx, nm, (unsigned long)addr, (unsigned long)size);
    }
    /* --- fim do DEBUG temporario --- */
                                    ...
                                    uint32_t task_base)
{
    ...
    f_lseek(fd, shstrndx * shentsize + shoff);
    if (f_read(fd, hdr_buf, sizeof(hdr_buf), &bytesRead) != FR_OK || bytesRead != sizeof(hdr_buf))
        return;
    strtab_offset = ((uint32_t)hdr_buf[0x10] << 24) | ((uint32_t)hdr_buf[0x11] << 16) |
                    ((uint32_t)hdr_buf[0x12] << 8)  |  (uint32_t)hdr_buf[0x13];

    for (uint16_t i = 0; i < shnum; i++) {
        f_lseek(fd, i * shentsize + shoff);
        if (f_read(fd, hdr_buf, sizeof(hdr_buf), &bytesRead) != FR_OK || bytesRead != sizeof(hdr_buf))
            continue;
        ...
        char name_buf[16];
        unsigned int nread;
        f_lseek(fd, strtab_offset + name_idx);
        f_read(fd, name_buf, sizeof(name_buf) - 1, &nread);
        name_buf[nread] = 0;
        ...
        for (uint32_t r = 0; r < num_relas; r++) {
            f_lseek(fd, rela_offset + r * 12);
            if (f_read(fd, rela_buf, 12, &bytesRead) != FR_OK || bytesRead != 12)
                continue;
...
static void apply_data_relocationsq(el_ctx *ctx, shdr_table_t *t, uint32_t task_base)
{
    for (uint16_t i = 0; i < t->shnum; i++) {
        const char *name = shdr_name(t, i);
